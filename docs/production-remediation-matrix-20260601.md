...
| GitHub workflow-aware preflight | workflow 文件变更时需要证明 token 具备 `workflow` scope；Actions 运行时弃用告警会影响后续 CI 稳定性 | 保持 `gh` auth 可用；失效时恢复 `gh auth` 或依赖 `git push --dry-run` fallback；workflow 文件变更时优先确保 `workflow` scope；CI、strict release 和 prod-to-pre sync workflow 主动设置 `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` | 普通审计已通过，workflow smoke 已固化 Node 24 Actions runtime opt-in | `npm run smoke:workflows`、`npm run github:push:preflight` 通过 |
