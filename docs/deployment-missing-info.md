...
当前本机检查结果：`cloudbase`、`tcb`、`tccli`、`docker`、`pg_dump`、`pg_restore`、`psql` 均未安装或不在 `PATH`，且当前 shell 未提供 `TENCENTCLOUD_SECRET_ID` / `TENCENTCLOUD_SECRET_KEY`。这不影响代码、构建、smoke 或基于 Node `pg` 的数据库迁移脚本；实际推送云环境或执行 prod 到 pre 数据同步时再补 CLI、PostgreSQL 同步工具与部署凭据。
