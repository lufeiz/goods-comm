...
当前审计快照：普通生产审计仍为 `BLOCKED (48 blockers, 10 warnings)`，严格生产审计仍为 `BLOCKED (50 blockers, 8 warnings)`。如果本机生产审计提示 GitHub CLI auth 不可用，普通代码/文档推送仍可依赖 Git 凭据和 `git push --dry-run` fallback；包含 workflow 文件的推送应先执行 `gh auth login` 或 `gh auth refresh -h github.com -s workflow` 恢复 workflow-aware preflight 证据。
