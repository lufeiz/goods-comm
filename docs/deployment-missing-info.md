...
- `npm run smoke:environment-commands`：检查 dev/test/pre/prod 四环境的 env check、后端启动、三端构建和数据库迁移 plan/execute npm scripts 是否齐全，并检查 pre/prod 部署与部署后 smoke 脚本是否保留；`verify:release` / `verify:release:strict` 已接入该检查。
