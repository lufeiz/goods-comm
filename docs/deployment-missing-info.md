...
- `GOODS_COMM_DB_MIGRATE_CONFIRM=migrate-pre npm run db:migrate:pre`：真实执行对应环境数据库 schema 初始化，dev/test/pre/prod 都有同名 `db:migrate:<env>` 脚本；执行要求真实连接串和 `psql`，生产迁移还必须额外设置 `GOODS_COMM_DB_MIGRATE_ALLOW_PROD=true`。
