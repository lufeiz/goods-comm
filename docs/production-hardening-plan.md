...
已生成后端部署产物 `dist/backend`，并提供 `backend/deploy/Dockerfile`、`backend/deploy/cloudbase.json` 和腾讯云部署说明。真实部署仍缺少的信息已经整理到 `docs/deployment-missing-info.md`；当前可先用 `.env.dev/test/pre/prod` 的占位值推进构建、环境校验和部署脚本开发。数据库迁移脚本已改为使用项目依赖 `pg` 直接执行，不再要求本机安装 `psql`；当前本机仍未安装 `cloudbase`、`tcb`、`tccli`、`docker`、`pg_dump`、`pg_restore` 或用于 prod-to-pre 同步 SQL 步骤的 `psql`，也没有可用云账号凭据、腾讯地图 Key 或正式社区网格数据，因此本轮无法直接推送到微信云托管 / 腾讯云或执行真实 prod 到 pre 数据同步。下一次部署需要先准备云环境 ID、合法 HTTPS 域名、数据库实例、prod-to-pre 同步所需 PostgreSQL 客户端工具、地图服务 Key、正式社区网格数据，以及具备 CloudBase / TencentDB / COS / CDN / TEM 权限的 `TENCENTCLOUD_SECRET_ID` / `TENCENTCLOUD_SECRET_KEY`。
