...
阻塞原因不是当前代码主链路失败，而是缺真实云资源、真实密钥、真实数据库 / 对象存储 / 地图服务 / 平台账号、生产告警 Webhook 和部署工具链。2026-05-29 续做后，`GOODS_COMM_MAP_REGION_DATASET` 已从数据集标签改为后端可解析的 JSON 数组占位，pre/prod 拓扑一致性仍通过；2026-05-30 续做后，pre/prod 结构化访问日志已纳入环境校验和部署后 health smoke。正式上线仍要替换为真实社区 / 街道网格数据，并接好云侧日志采集和保留策略。
...
- 2026-05-31 续做后，首页无可用定位时不再混用“暂无匹配物品”空状态，而是展示独立的定位引导状态，提供刷新定位和选择位置入口；有效定位但确实没有可交易商品时才展示普通空列表。
...
- 2026-05-30 续做后，后端新增 `GOODS_COMM_ACCESS_LOG_ENABLED` 结构化访问日志开关；pre/prod 默认开启并写 stdout JSON，日志只记录 trace id、归一化路由、状态码、耗时、CORS 和限流摘要，不记录 query、请求头、请求体或动态业务 ID；`/health` / `/health/ready` 会暴露 `accessLog`，部署后 health smoke 在 pre/prod 会断言 `accessLog.enabled=true`。
...
- 2026-05-30 续做后，新增 `npm run github:push:preflight` 和 `smoke:github-push-preflight`，检查 `origin=https://github.com/lufeiz/goods-comm`、`main` upstream 和工作区干净；待推送提交包含 `.github/workflows/*.yml` / `.yaml` 时再强制检查 GitHub CLI token 的 `repo` / `workflow` scope，普通代码 / 文档变更在 `gh` auth 不可用时只 warning，避免本机 Git 凭据可推送但 `gh` 会话失效导致夜间推送误阻断。每天 21:00 的 `goods-comm-nightly-github-push` 自动化会在 quick release gate 通过后再执行该 preflight 并推送 `origin/main`。
...
- 2026-05-31 续做后，新增 `scripts/deploy-frontend.mjs`、`npm run deploy:frontend:*` 和 `npm run smoke:frontend-deploy`，把 H5 CloudBase 静态托管、微信开发者工具上传、支付宝小程序上传的前端部署前置项和执行保护纳入 release gate；full/release profile 会输出 pre/prod 前端部署计划。
