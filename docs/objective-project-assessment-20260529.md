...
图片上传和商品文本审核现在都会先解析服务端 session，再把 session 绑定用户的微信 `openid` 传给内容安全适配器。客户端提交的 `sellerOpenid`、`platformId` 或类似审核身份字段会被移除，既不会进入微信审核 payload，也不会写入商品响应。后端 smoke 已覆盖图片审核 openid、文本审核 openid、客户端伪造审核身份剥离和幂等重放不重复审核。
