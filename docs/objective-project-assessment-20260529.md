...
| 后端 / 数据 / 安全 | 66/100 | 后端边界、schema、幂等、适配器、请求体上限、基础安全响应头、客户端 IP 限流、接口级配额、认证主体写请求配额、可信代理白名单、结构化访问日志、运营登录锁定和 fail-closed 意识较好；发布重试已能在外部文本审核前命中幂等重放，审核 `openid` 已绑定服务端 session；PostgreSQL snapshot rewrite 已用事务级 advisory lock 串行化；真实 DB、凭据、边缘 WAF、分布式限流、云侧日志采集和增量仓储不足。 |
| 测试 / 发布 / 运维 | 60/100 | release gate、artifact smoke、访问日志 smoke、页面契约 smoke、主流程证据矩阵、H5 渲染主链路 smoke 和 strict workflow 较完整；真实部署后 smoke、微信/支付宝真机矩阵、工具链和真实云资源仍 blocked，普通门禁容易被误读为上线门禁。 |
...
| `backend/src/server.mjs:22-80` | Node HTTP 后端装配 state store、平台登录、对象存储、内容安全、区域解析、平台通知、生产告警、结构化访问日志、运营鉴权和限流器。 |
| `backend/src/server.mjs:117-193` | `/health` 和 `/health/ready` 能暴露运行环境、依赖状态、`opsAlert` 告警配置和 `accessLog` 开关，适合部署后验收。 |
...
| `backend/src/request-logger.mjs` / `scripts/request-logger-smoke.mjs` / `scripts/deployed-health-smoke.mjs` | 后端已有 stdout JSON 访问日志，默认 pre/prod 开启；日志记录 trace id、归一化路由、状态、耗时、CORS 和限流摘要，不记录 query、请求头、请求体或动态业务 ID；部署后 health smoke 会断言 pre/prod `accessLog.enabled=true`。 |
...
本轮实际执行 `npm run smoke:pages`、`npm run smoke`、`npm run smoke:main-flow-contract`、`npm run smoke:h5:render` 和 `npm run verify:release:quick -- --skip-http-backend` 通过，其中 quick release gate 已接入 H5 渲染主链路 smoke。2026-05-30 续做后新增主流程证据矩阵，把登录协议与账号生命周期、定位解析与显示可信边界、发布与图片上传、交易售卖生命周期、部署后 smoke 和 release gate 串成 5 条主链路证据；同时新增 CDP 驱动的 H5 渲染 smoke，真实启动构建产物并验证登录、浏览器定位显示、发布、交易和售卖状态流。H5 Browser 渲染 QA 也覆盖了首页首屏、搜索交互、移动视口，以及首页无定位时不再出现 `Cannot read properties of null (reading 'latitude')`。后续又新增生产告警适配器、结构化访问日志、`smoke:ops-alerts` 和 `smoke:request-logger`，quick release gate 当前完成 `98/98` 项；同时刷新普通和严格生产审计，当前仍分别是 `BLOCKED (50 blockers, 9 warnings)` 与 `BLOCKED (52 blockers, 7 warnings)`，剩余 blocker 仍来自真实云资源、真实密钥、工具链、告警 Webhook URL/token 和部署后 smoke 输入等上线前置输入。
...
| 测试 / 发布 / 生产就绪 | 60/100 | npm scripts、smoke、访问日志 smoke、主流程证据矩阵、H5 渲染主链路 smoke、release gate 和 strict workflow 完整度较好；真实部署、部署后 smoke、工具链、微信/支付宝真机矩阵仍 blocked，且普通 CI 绿色不能代表可上线。 |
...
5. `/health/ready` 在 pre/prod 均通过，且实际使用 `postgres`、`cos`、`tencent` 地图、`wechat` 内容安全和通知、`webhook` 生产告警，并保持 `accessLog.enabled=true`。
...
8. 网关/WAF、分布式限流、实际可信代理 IP / 网段、stdout 访问日志采集、日志保留策略和值班告警策略配置完成，并在真实云侧跑通部署后 health smoke，确认安全响应头、`opsAlert=webhook` 和 `accessLog.enabled=true` 未被代理或环境变量覆盖。
