...
assert.equal(rows.locationRiskEvents.every((event) => Boolean(event.reviewStatus)), true)
assert.equal(rows.locationRiskEvents.filter((event) => event.reviewStatus === 'not_required').length >= 1, true)
...
assert.ok(rows.sessions.find((session) => session.userId === sellerLogin.user.id).lastSeenAt >= rows.sessions.find((session) => session.userId === sellerLogin.user.id).createdAt)
...
assert.ok(restored.sessions.find((session) => session.userId === sellerLogin.user.id).lastSeenAt >= restored.sessions.find((session) => session.userId === sellerLogin.user.id).createdAt)
...
assert.equal(restored.locationRiskEvents.every((event) => Boolean(event.reviewStatus)), true)
...
assert.equal(roundTripRows.sessions.length, rows.sessions.length)
assert.ok(roundTripRows.sessions.find((session) => session.userId === sellerLogin.user.id).lastSeenAt >= rows.sessions.find((session) => session.userId === sellerLogin.user.id).createdAt)
...
assert.equal(roundTripRows.locationRiskEvents.every((event) => Boolean(event.reviewStatus)), true)
