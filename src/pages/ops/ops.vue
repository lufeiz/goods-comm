  ...
  <view class="page">
    <view class="panel">
      <view class="panel-head">
        ...
        <text :class="['state-pill', hasAuth ? 'ready' : 'blocked']">{{ hasAuth ? '已登录' : '待登录' }}</text>
      </view>
      ...
      <view class="button-row">
        <button class="mini-button primary" @tap="saveSecret">保存</button>
        <button class="mini-button" :disabled="!hasAuth || loading" @tap="refresh">刷新</button>
        <button class="mini-button danger" @tap="clearSecret">清除</button>
      </view>
      <text v-if="opsSession?.operator?.id" class="case-meta">当前操作人：{{ opsSession.operator.id }}</text>
    </view>

    <view v-if="queue" class="stats-grid">
    ...
    <view class="panel">
      ...
      />
      <view class="button-row">
        <button class="mini-button danger" :disabled="loading || !userRiskTarget" @tap="updateUserStatus('blocked')">封禁</button>
        <button class="mini-button" :disabled="loading || !userRiskTarget" @tap="updateUserStatus('active')">解封</button>
      </view>
      ...
      <view v-if="users.length" class="case-list">
        <view v-for="user in users" :key="user.id" class="case-card">
          ...
          <view class="button-row">
            <button class="mini-button" :disabled="loading" @tap="prefillUserRisk(user)">填入</button>
            <button v-if="user.status === 'blocked'" class="mini-button" :disabled="loading" @tap="updateListedUserStatus(user, 'active')">解封</button>
            <button v-else class="mini-button danger" :disabled="loading" @tap="updateListedUserStatus(user, 'blocked')">封禁</button>
          </view>
        </view>
      </view>
      <text v-else class="empty-text">暂无匹配用户</text>
    </view>
    ...
    <view class="panel">
      ...
      <view v-if="pendingItems.length" class="case-list">
        <view v-for="item in pendingItems" :key="item.id" class="case-card">
          <view class="case-head">
          ...
          />
          <view class="button-row">
            <button class="mini-button primary" :disabled="loading" @tap="reviewItem(item, 'approved')">通过</button>
            <button class="mini-button danger" :disabled="loading" @tap="reviewItem(item, 'rejected')">拒绝</button>
          </view>
        </view>
      </view>
      <text v-else class="empty-text">暂无待审商品</text>
    </view>
    ...
    <view class="panel">
      ...
      <view v-if="reports.length" class="case-list">
        <view v-for="report in reports" :key="report.id" class="case-card">
          <view class="case-head">
          ...
          />
          <view class="button-row">
            <button class="mini-button" :disabled="loading" @tap="resolveReport(report, 'dismiss_report')">驳回</button>
            <button class="mini-button danger" :disabled="loading" @tap="resolveReport(report, 'uphold_report')">确认违规</button>
            <button class="mini-button danger" :disabled="loading || !report.reporter?.id" @tap="blockReporter(report)">封禁举报人</button>
          </view>
        </view>
      </view>
      <text v-else class="empty-text">暂无待处理举报</text>
    </view>
    ...
    <view class="panel">
      ...
      <view v-if="disputes.length" class="case-list">
        <view v-for="dispute in disputes" :key="dispute.id" class="case-card">
          <view class="case-head">
          ...
          />
          <button class="full-button primary" :disabled="loading" @tap="resolveDispute(dispute)">提交处理</button>
        </view>
      </view>
      <text v-else class="empty-text">暂无争议交易</text>
    </view>
    ...
    <view class="panel">
      ...
      <button class="full-button" :disabled="!hasAuth || loading" @tap="retryDueDeliveries">重试到期异常</button>
      <view v-if="notificationDeliveries.length" class="case-list">
        <view v-for="delivery in notificationDeliveries" :key="delivery.id" class="case-card">
          <view class="case-head">
          ...
          <button class="full-button primary" :disabled="loading" @tap="retryDelivery(delivery)">立即重试</button>
        </view>
      </view>
      <text v-else class="empty-text">暂无通知投递记录</text>
    </view>
    ...
    <view class="panel">
      ...
      <view v-if="clientEvents.length" class="case-list">
        <view v-for="event in clientEvents" :key="event.id" class="case-card">
          <view class="case-head">
      ...
      <text v-else class="empty-text">暂无端侧事件</text>
    </view>
    ...
    <view class="panel">
      ...
      <view v-if="auditEvents.length" class="case-list">
        <view v-for="event in auditEvents" :key="event.id" class="case-card">
          <view class="case-head">
      ...
      <text v-else class="empty-text">暂无操作审计</text>
    </view>
  </view>
</template>
